...
namespace AlexaNetCore.Model
{
    ...
    public class AlexaSlotValidationIsNotInDuration<T> : IAlexaSlotValidation
    {
        ...
        public AlexaSlotValidationIsNotInDuration(T start, T end, string p)
        {
        ...
        public AlexaSlotValidationIsNotInDuration(T start, T end, AlexaMultiLanguageText p)
        {
            Prompt = new AlexaPrompt(Guid.NewGuid().ToString(), p);
            Start = start;
            End = end;
        ...
        public AlexaSlotValidationIsNotInDuration<T> AddInvalidValuePromptVariation(string promptTxt)
        {
